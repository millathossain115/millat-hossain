...
export function getProjectsTravelDistance(track: HTMLElement): number {
...
export default function useProjectsAnimations({
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const headingEl = section.querySelector<HTMLElement>('.proj-heading');
      if (headingEl) {
      ...
      const labelEl = section.querySelector<HTMLElement>('.proj-label');
      if (labelEl) {
        gsap.fromTo(
          labelEl,
          { opacity: 0, y: 18 },
