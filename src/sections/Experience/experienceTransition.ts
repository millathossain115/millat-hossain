...
interface AddExperienceIntroProps {
  timeline: gsap.core.Timeline;
  educationSection?: HTMLElement | null;
  experienceSection: HTMLElement | null;
}

export function addExperienceIntroToTimeline({
  timeline,
  experienceSection,
}: AddExperienceIntroProps): (() => void) | undefined {
  ...
  const overlay = experienceSection.cloneNode(true) as HTMLElement;
  ...
  const headingZone = overlay.querySelector<HTMLElement>('.exp-heading-zone');
  const headingTitle = overlay.querySelector('.exp-heading-title');
  const list = overlay.querySelector<HTMLElement>('.exp-list');
  const cards = gsap.utils.toArray<HTMLElement>('.exp-work-item', overlay);
  const rows = cards.flatMap((card) =>
    gsap.utils.toArray<HTMLElement>('.exp-card-row, .exp-card-rule', card)
  );
  ...
  const realList = experienceSection.querySelector<HTMLElement>('.exp-list');
  const realCards = gsap.utils.toArray<HTMLElement>('.exp-work-item', experienceSection);
  ...
  const syncOverlayHandoff = () => {
    ...
    const introStart = (timeline.labels as Record<string, number>).experienceIntro ?? 0;
